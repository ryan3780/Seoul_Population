...
예를 들면, 원하는 동을 검색하기 위한 함수의 이름은 `getUniqueDongName`으로, 유일한 동의 이름을 가져온다라는 것을 내포한다.  
처음 함수의 명은 `selectedDongName`으로 했었다. 확실히, 예전 이름보다 현재의 이름이 가독성이 높다고 생각한다.  
다른 예시로는, 선택한 구의 통계자료를 보여주는 컴포넌트의 이름은 `GuInfo`이며, 해당 컴포넌트에서 단순히 View를 담당하는 배열의 이름을 `views`라고 했다.   
views라는 것은 기준과는 다르지만, 다른 곳에서 쓰이질 않고 정말, 단순 보여주기만 하기에, 가독성도 높여 준다고 생각한다.
#### `Component Divide`  
...
검색 기능을 하는 컴포넌트의 `state`를 어디서 선언을 하고, `props`를 어디로 내려 줄지, 고민을 하다가, App.js 정했다.  
아래에서 위로 가는 것보다, 위에서 아래로 뿌리는게 편하고, 효율성이 높다고 생각했다. `구`와`동`을 검색하는 InputBox 자체가 동일한 기능임을 보고  
1개의 공통 컴포넌트를 만들고, 각각 이름표를 다르게 붙여주면 더 좋지 않을까 해서, SearchBar.js & SearchBox.js가 만들어 졌다.
<br/>
더 좋은 컴포넌트 분리를 위해서  찾아낸 방법은 github에 올린 자료를 검색하여, 현재 만들고 있는 것과 `비슷한` 것 혹은 `가독성`이 좋은 사람의 코드를 보면서  
컴포넌트를 `어떻게` 분리를 했는지 눈으로 보고, 말하면서, 감탄하면서 현재 작업하는 것에 적용하려고 노력하고 있다.
#### `Remove Duplicated Code`  
중복 코드를 제거하는 것이 `가독성`과 `퍼포먼스`를 향상시키는 것이라고 생각한다.  
허나, 이것 역시 어렵다. 막상, `중복`코드를 찾아 냈는데, 이걸 어떻게 고쳐야 할지, 무슨 방법이 좋을지 고민한다.  
예를 들면, `Statistics.js`라는 컴포넌트는 원래 선택된 구와 동의 통계자료를 보여주는 단 1개의 컴포넌트 였다.  
하지만, 가독성과 관리를 위해서 동일한 형식의 반복문과 조건문이 쓰이기 때문에 `GuInfo`와`DongInfo` 2개의 컴포넌트로 나누었다.  
글을 쓰면서, 코드를 다시 읽어보는 중인데, 이것도 위에 `SearchBox`처럼 `InfoBox`처럼 공통의 컴포넌트를 만들어서 사용이 가능할 거 같다.  
<br/>
위 내용처럼 근래의 코딩에서는 기준을 계속 추가 하고 있다.  
