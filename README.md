...
다른 예시로는, 선택한 구의 통계자료를 보여주는 컴포넌트의 이름은 `GuInfo`이며, 해당 컴포넌트에서 단순히 View를  
담당하는 배열의 이름을 `views`라고 했다.   
...
아래에서 위로 가는 것보다, 위에서 아래로 뿌리는게 편하고, 효율성이 높다고 생각했다.  
`구`와`동`을 검색하는 InputBox 자체가 동일한 기능임을 보고  1개의 공통 컴포넌트를 만들고  
각각 이름표를 다르게 붙여주면 더 좋지 않을까 해서, SearchBar.js & SearchBox.js가 만들어 졌다.
  
더 좋은 컴포넌트 분리를 위해서  찾아낸 방법은 github에 올린 자료를 검색하여,   
현재 만들고 있는 것과 `비슷한` 것 혹은 `가독성`이 좋은 사람의 코드를 보면서    
